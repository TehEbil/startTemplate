...
.controller('multiSelectCtrl', ["$scope", "$compile", "$timeout", "$element", "$attrs", "$parse", function($scope, $compile, $timeout, $element, $attrs, $parse) {
  ...
  console.log($scope.col.filters)
  console.log($scope.col.filters[0].selectOptions)
  $scope.colFilter = $scope.col.filters[0];
  $scope.title = col.field;
  $scope.showModal = function() {
    $scope.listOfTerms = [];
    uneditedTerms = $scope.col.filters[0].selectOptions;
    for(var key_s in uneditedTerms)
      $scope.listOfTerms.push(uneditedTerms[key_s].value)
    ...
    var html = '<div class="ui-grid-multi-select-filter"><div class="modal-body"><div class="multi-select-grid" ui-grid="gridOptions" ui-grid-selection></div></div><div class="footer"><button class="btn-close" ng-click="close()">Filter</button><button class="btn-close" ng-click="cancel()">Cancel</button></div></div>';
    $elm = angular.element(html);
    var x = $element[0].closest('.ui-grid-new-filter');
    console.log(x);
    angular.element(x).append($elm);
